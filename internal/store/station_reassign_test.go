...
// *** THE ABANDONED-WORKSPACE DEAD END, AND THE ONLY DOOR OUT OF IT. ***
//
// A session adopts a workspace only while `session_key IS NULL` — deliberately, because a key that
// could take a claimed post would be a credential, and it is documented as selecting rather than
// authorising. The consequence nobody had answered: when a conversation dies, the workspace it
// claimed is SEALED. Its notes, tasks, locker and vault are intact and unreachable forever.
//
// This asserts the recovery as a human would experience it, end to end: point the workspace at a
...
func TestReassignLetsANewConversationTakeOverAnAbandonedWorkspace(t *testing.T) {
	st, ctx, actorID := stationFixture(t)

	// The dead conversation's workspace, with work in it.
	...
	// CONTROL FIRST: without a reassign, the new conversation gets a DIFFERENT workspace. This is
	...
	if !created || stranded.StationID == original.StationID {
		t.Fatalf("a fresh conversation reached the abandoned workspace with no human action "+
	...
	if created {
		t.Error("the takeover minted a NEW workspace instead of returning the recovered one")
	}
	if got.StationID != original.StationID {
		t.Fatalf("the session landed in %q, want the recovered workspace %q", got.StationID, original.StationID)
	}
	...
	if len(notes) != 1 || notes[0].Key != "handoff" {
		t.Errorf("the recovered workspace holds %v, want the handoff page that made it worth "+
			"recovering", notes)
	}
}

// AN EMPTY KEY RELEASES, and that is not a convenience. Without it a workspace pointed at the
// wrong conversation is stuck to it permanently — the same dead end, re-created by the fix for it.
func TestReassignWithAnEmptyKeyReturnsTheWorkspaceToThePool(t *testing.T) {
	st, ctx, actorID := stationFixture(t)
	original, _, err := st.ClaimStationForSession(ctx, "conv-dead", "old laptop", actorID, "")
	if err != nil {
		t.Fatal(err)
	...
	if released.Station.SessionKey != "" {
		t.Errorf("the workspace still answers to %q after being released", released.Station.SessionKey)
	}
	...
	if created || got.StationID != original.StationID {
		t.Errorf("a released workspace could not be adopted (created=%v, id=%q, want %q)",
	...
	if _, err := st.StationBySessionKey(ctx, "conv-dead"); !errors.Is(err, ErrNotFound) {
		t.Errorf("the released key still resolves to a workspace: %v", err)
	}
}
...
// already called station_me, so a fresh empty workspace already answers to the key it just
...
//
// Nothing is destroyed by taking it: the displaced workspace keeps everything and stays listed.
// So the safety is DISCLOSURE — the result must name what it displaced, or this is a silent steal.
func TestReassignTakesTheKeyFromTheWorkspaceHoldingItAndSaysSo(t *testing.T) {
	st, ctx, actorID := stationFixture(t)
	abandoned, _, err := st.ClaimStationForSession(ctx, "conv-dead", "old laptop", actorID, "")
	if err != nil {
		t.Fatal(err)
	}
	// The fresh workspace the taking-over session already minted for itself.
	...
	if res.TakenFromID != fresh.StationID {
		t.Errorf("the result says the key came from %q, want the workspace that held it, %q — "+
	...
	// THE DISPLACED WORKSPACE IS UNCLAIMED, NOT DAMAGED: nothing else may reach it by the old key,
	...
	if got, err := st.StationBySessionKey(ctx, "conv-new"); err != nil {
		...
		t.Errorf("the key still resolves to %q, want the workspace it was moved to", got.StationID)
	}
	if still, err := st.StationByID(ctx, fresh.StationID); err != nil {
		t.Fatalf("the displaced workspace is gone: %v", err)
	} else if still.SessionKey != "" {
		t.Errorf("two workspaces answer to one key (%q still holds %q)", still.StationID, still.SessionKey)
	}
}

// RE-POINTING A WORKSPACE AT THE KEY IT ALREADY HAS IS NOT A COLLISION WITH ITSELF. An operator
// who opens the form, reads the prefilled key and clicks Reassign must not be told the key is
// taken by the very workspace they are editing.
func TestReassignToItsOwnKeyIsANoOp(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("a workspace could not be reassigned to the key it already holds: %v", err)
	}
	if got.Station.SessionKey != "conv-a" {
		t.Errorf("the workspace answers to %q afterwards, want conv-a", got.Station.SessionKey)
	}
}
...
// workspace they had shut, with nothing to explain it.
func TestReassignRefusesAnArchivedWorkspace(t *testing.T) {
	st, ctx, actorID := stationFixture(t)
	...
	if _, err := st.ReassignStationToSession(ctx, dead.StationID, "conv-x"); err != nil {
		t.Fatalf("an active workspace refused a reassign: %v", err)
	}
	...
	if _, err := st.ReassignStationToSession(ctx, dead.StationID, "conv-y"); err == nil {
		t.Error("an ARCHIVED workspace accepted a reassign — archive would be advisory")
	}
}
