...
var Fields = []Field{
	...
	intField("comm_metadata_ttl_sec", "Inter-session comms", "Metadata retention (seconds)",
		"How long a settled message's row survives after it SETTLED — the last recipient's acknowledgement, or expiry for one nobody read. (It was measured from CREATION until 3.6.0, so a message that expired on day 30 had already spent its retention before anyone could have read it.) NOTE: since bodies now survive acknowledgement (see Body retention), this is also the ONLY bound on a body nobody ever read. Raising this for a longer audit trail multiplies retained bytes with it.",
