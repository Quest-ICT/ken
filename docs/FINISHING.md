...
> **STAGED, NOT RELEASED: 4.0.0, THE BREAKING WAVE.** Seventeen commits since 3.42.0, net −6,377
> lines: one machine surface, station keys retired, both human gates on comm removed, a mailbox that
> belongs to a station, the directory opened to the estate, per-tool rules moved into
> `ken_instructions`, and "workspace" retired in favour of **station** everywhere. The entry is under
> `## [Unreleased]` in `CHANGELOG.md` and the operator briefing is in `docs/UPGRADING.md`. This
> paragraph names the newest RELEASED version below, because that is what the gate checks and what
> an operator is actually running.
>
