...
**DECIDED 2026-08-24: DELETE.** The three functions are gone; the `DROP TABLE` ships as its own release under Rule 4.

**Two things decided it, and neither was available to the earlier passes.** First, the deny is not merely unwired but **unenforceable from where the sends happen**: `comm.Store` holds handles to comm.db alone — no ken.db handle — and comm.db has no block mirror, while links and rooms each have one. Measured: zero references to `station_block` anywhere in `internal/comm/`, against 6 and 11 for `station_link_mirror` and `room_member_mirror` on the identical search. So "wire it honestly" is a cross-database projection change, not the console-plus-check this entry costed it as. Second, **Vlad's own rule in `DECISIONS-BATCH5.md:176` already answered it** — *"If P2 is chosen with the link requirement intact, the default is not permissive and `station_block` stays optional"* — and P2 shipped with the link requirement intact. Nobody connected the two for a week.

**THE GAP THIS LEAVES OPEN, recorded here because it is real and outlives the code.** The capability is NOT superseded. Revoking a link kills only `to_station`; `roomMembers` and the broadcast union carry no link predicate, and `AddRoomMember` requires no link to exist, so two stations with no link at all still reach each other through any shared room and through `to_room:"all"`. Archiving stops everything but retires the post. **To stop one pair, an operator must still remove a station from a room and cost it that room's other relationships** — which is precisely the "narrow something wide" that migration 0017's comment named. If that ever becomes a real operational need, the shape is a comm.db block mirror plus a `NOT EXISTS` inside the comm writer transactions, and `IDENTITY.md` §9.5 constrains how it may be justified: under one human a pair deny is an **intent** mechanism and a blast-radius limiter, never an isolation boundary.
...
**Closed, and the last measurement of it is banked.** ken-prod-ops ran `SELECT COUNT(*) FROM station_block` on the live database before the drop and got **0** — as predicted, so no hidden write path ever existed. That figure is now the ONLY measurement of that table that will ever exist anywhere, which is why it is recorded here rather than only in a message. They also verified the FK direction against their own schema rather than accepting mine: both station columns are `ON DELETE CASCADE` toward `station`, so the cascade fires when a STATION is deleted and dropping `station_block` cannot reach it. Baselines for the after-check: station 10, actor 6, station_link 3.

**And the capability was confirmed unwanted THERE, with a reason that does not generalise.** Production has never wanted a pair deny — checked against their message log, not from impression — but every station on that deployment answers to one human (`admin`), so divergent interests cannot arise. Their "no" is evidence about that instance and not about the feature. The one time they did want to stop a pair (the `proxmox-servers` test link, 2026-08-19) **revoking the link was exactly the right lever** and did precisely what was needed. That instance is the strongest argument for the deletion, because it is a case rather than an absence. Where they can see it biting: their one room has three members, and silencing a pair inside it would still cost two other relationships.

Slice 7's H4 blocker is dissolved. Worth keeping the history on this one: it survived **two** batches by being neither wired nor dropped, its deferral was explicitly worded "with a date on it", the date passed unremarked, and it was finally rediscovered by a sweep looking for something else entirely. `FINISHING.md:49` already cited this same table as the project's example of an item that vanishes into a closed batch — and it then did it a second time.
