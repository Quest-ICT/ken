...
- **Amended 2026-08-24 (3.20.0):** this column is also a WELD. It is checked at use on every call, so retiring the key ends its bound sessions even where nothing severed them — and docs/IDENTITY.md §9.3 called bound endpoints the safe ones. `/comm` now renders it and can move a binding to another key of the SAME station, which is the whole of the rotation story for it. Measured on production the same day: 8 live bound endpoints, 8 distinct keys, 1:1 — an accident of provisioning rather than a guarantee.
