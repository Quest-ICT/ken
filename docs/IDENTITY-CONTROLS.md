...
- **Amended 2026-08-24 (3.20.0):** this column is also a WELD. It is checked at use on every call, so **revoking** the key — or deleting its row — ends its bound sessions even where nothing severed them. (Not *retiring*: `IsStationKeyRevoked` reads `revoked_at` alone, so a retired key stops the station surface and leaves these sessions running, exactly as the `retired_at` entry above and `stations.key_retire_help` state. The first version of this amendment said retire, and two shipped locale strings said it too — the same conflation the `retired_at` entry records, in the opposite direction.) — and docs/IDENTITY.md §9.3 called bound endpoints the safe ones. `/comm` now renders it and can move a binding to another key of the SAME station, which is the whole of the rotation story for it. Measured on production the same day: 8 live bound endpoints, 8 distinct keys, 1:1 — an accident of provisioning rather than a guarantee.
